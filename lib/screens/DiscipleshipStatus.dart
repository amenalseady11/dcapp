...
disciple.DiscipleshipStatusClass discipleclass = new disciple.DiscipleshipStatusClass();
List<disciple.Discipleshipstatus> disc = new List<disciple.Discipleshipstatus>();
List<disciple.Discipleshipstatus> filteredDisciple = new List<disciple.Discipleshipstatus>();
List<disciple.DiscipleshipStatusClassTraining> alltraining = new List<disciple.DiscipleshipStatusClassTraining>();
List<disciple.DiscipleshipstatusTraining> disctrain = new List<disciple.DiscipleshipstatusTraining>();
...
//DiscipleshipStatusClassTraining dtrain = new DiscipleshipStatusClassTraining();
  ...
  void initState() {
    ...
    setState(() {
     ...
     DiscipleshipStatuservice.getDiscipleshipStatus(global.profile.member.branch.branchId).then((discipleFromServer) {
        setState(() {
   ...
   getIndex(int index){
    ...
    getmemberPhone(int memberId){
      ...
      var phone = global.members.members.where((u)=>u.memberId== memberId).toList();

      if(phone.length<=0){
        return Column(
          children: <Widget>[
            Text('Phone Number not available or zone not selected', style: TextStyle(color: Colors.red, fontSize:10),),
          ],
        );
      }
      return
       Row(
         children: <Widget>[
            new IconButton
                           ...
                           Text(phone[0].phoneNumber, style: TextStyle(fontSize:18),),
         ],
       );
    }
                  ...
                  children: <Widget>[
                    Row( mainAxisAlignment: MainAxisAlignment.center,children: <Widget>[
                     ...
                     Row( mainAxisAlignment: MainAxisAlignment.center,children: <Widget>[
                      ...
                      Text(filteredDisciple.length.toString(), style: TextStyle(color: Colors.white, fontSize: 25, fontWeight: FontWeight.bold),),
                    ],),
                    SizedBox(height:30),
                     Container(
                    ...
                    setState(() {
                    filteredDisciple = disc.where((u)=>
                    (u.memberName.toLowerCase().contains(string.toLowerCase())||
                
                    (u.training.toString().toLowerCase().contains(string.toLowerCase()))))
                    .toList();
                    });
                            ...
                            children: <Widget>[
                                ...
                                crossAxisAlignment: CrossAxisAlignment.start,
                                         children: <Widget>[
                                           Row(
                                             children: <Widget>[
                                               Column(
                                                 children: <Widget>[
                                                   Text(filteredDisciple[index].memberName , 
                                                   style: TextStyle(
                                                        fontSize: 18.0,
                                                        fontWeight: FontWeight.bold,
                                           ...
                                           Row(
                                             ...
                                             getmemberPhone(filteredDisciple[index].memberId)
                                           ])
                                         ],
                                       ),
                                 ...
                                 SizedBox(height:2),
                                   Padding(
                                     ...
                                     child: Row(
                                       children: <Widget>[
                                         Text('Discipleship Level:', style: TextStyle(fontWeight: FontWeight.bold),),
                                         SizedBox(width: 20),
                                         Row(
                                          children: <Widget>[
                                          
                                           Text(getIndex(index).toString() +' %', style: TextStyle(fontSize:20),)
                                         ]
                                  ),
                                       ],
                                     ),
                             ...
                             Column(
                              ...
                              Navigator.push(context, MaterialPageRoute(builder: (context){
                                     global.membername = filteredDisciple[index].memberName;
                                global.memberId =filteredDisciple[index].memberId;
